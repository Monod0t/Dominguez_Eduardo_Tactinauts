...
public class LobbyController : MonoBehaviour
{
    ...
    [Header("Other")]
    [SerializeField] GameObject lobbyDataPrefab;
    ...
    public void StartGameProtocol()
    {
        if (lobbyData == null)
        {
            LobbyData lobbyDataFinder = FindObjectOfType<LobbyData>();
            if (lobbyDataFinder == null)
            {
                GameObject instantiatedLobbyData = Instantiate(lobbyDataPrefab, transform.position, Quaternion.identity);
                lobbyData = instantiatedLobbyData.GetComponent<LobbyData>();
            }
        }

        lobbyData.UpdatePlayerData(currentPlayerCount, lobbySlotsList);
